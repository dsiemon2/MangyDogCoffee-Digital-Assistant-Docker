# Setup Guide - Mangy Dog Coffee Digital Voice Assistant

This guide walks you through setting up all required services: Twilio (telephony) and OpenAI (voice AI).
...
3. Name it (e.g., "Mangy Dog Coffee Voice Assistant")
...
```sql
CREATE DATABASE mangydogcoffee;
CREATE USER mangydogcoffee WITH PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE mangydogcoffee TO mangydogcoffee;
```
...
```env
DATABASE_URL=postgresql://user:password@localhost:5432/mangydogcoffee
...
4. Manage knowledge base (across 24 languages)
5. Configure voice selection (8 voices) and language settings
6. Customize greeting message with preview
7. View analytics
...
DATABASE_URL=postgresql://user:pass@prod-host:5432/mangydogcoffee
...
**Azure App Service:**
```bash
az webapp up --name mangy-dog-coffee --resource-group your-rg
```
