...
This document outlines a proposal to enhance the Mangy Dog Coffee Digital Assistant by offloading complex business logic and integrations to **n8n** (an AI-native workflow automation platform).
...
graph LR
    ...
    Twilio -- Stream --> Server[MDC Assistant Server]
...
2.  **Scrape**: Fetch content from `mangydogcoffee.com/faq`, `/about`, `/products`.
3.  **LLM Extract**: Use an LLM node to extract Q&A pairs and format them as Markdown.
4.  **API Call**: Call the MDC Assistant's Admin API (or write to DB directly) to update the Knowledge Base documents.
