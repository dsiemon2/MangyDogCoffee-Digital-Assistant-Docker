...
// Routes (Admin routes moved to adminServer.ts on port 8030)
app.use('/healthz', healthRouter);
app.use('/', twilioWebhook);
app.use('/stripe', stripeWebhook);
app.use('/', localTestRouter);

const port = process.env.PORT ? Number(process.env.PORT) : 8031;
...
server.listen(port, () => {
  logger.info(`Mangy Dog Coffee Voice Assistant running on :${port}`);
